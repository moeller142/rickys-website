...
export default function Header({tabInfo}) {
    ...
    const topMenu = (tabs) => tabs.map(tab => <LinkedTab colorStyle='dark' link={tab.link} title={tab.title}/>);

    const sideMenu = (tabs) => <MenuIcon onClick={() => openSideMenuOpen(true)} className='tab' fontSize='large'/>;
    ...
    return(
        ...
        <Drawer anchor='right' open={sideMenuOpen} onClose={() => openSideMenuOpen(false)}>
                <div className="hamburger-menu">
                    {tabInfo.map(tab => <LinkedTab link={tab.link} title={tab.title} colorStyle='light'/>)}
                </div>
            </Drawer>
